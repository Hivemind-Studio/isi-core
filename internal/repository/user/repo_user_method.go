...
func (r *Repository) Create(ctx context.Context, tx *sqlx.Tx, name string, email string,
	...
	insertUserQuery := `INSERT INTO users (name, useremail, password, role_id, phone_number, status, verification) VALUES (?, ?, ?, ?, ?, ?, ?)`
...
func (r *Repository) CreateStaff(ctx context.Context, tx *sqlx.Tx, user User) (id int64, err error) {
	...
	insertUserQuery := `INSERT INTO users (name, useremail, password, role_id, phone_number, status, 
...
func (r *Repository) GetByVerificationTokenAndEmail(ctx context.Context,
	...
	query := `SELECT id, useremail, verification_token, trial, expired_at, created_at, updated_at
			  FROM email_verifications 
			  WHERE verification_token = ? AND useremail = ?`
...
func (r *Repository) DeleteEmailTokenVerification(ctx context.Context, tx *sqlx.Tx, email string) error {
	query := `DELETE FROM email_verifications WHERE useremail = ?`
